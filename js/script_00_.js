...
// --------------------------------  LOGIN REALIZADO  ------------------------------------------- 
  ...
  if (nombrePirata) {
      greeting.textContent = `¡Ahoy, ${nombrePirata}!`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Comprobar si ya se guardó una decisión
    const decision = localStorage.getItem("cookies-accepted");

    if (!decision) {
      banner.style.display = "flex"; // Mostrar solo si no hay decisión
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
      ...
      console.warn("Error gestionando cookies:", e);
    }
  });
